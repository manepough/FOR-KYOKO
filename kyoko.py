...
import os, re, asyncio, json, urllib.parse

# ── config ─────────────────────────────────────────────────────────────────────
DISCORD_TOKEN  = os.getenv("DISCORD_BOT", "")
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "")
AZURE_BOT_ID   = "1492835775241392189"
...
async def on_message(message):
    ...
    # stop command from owner or auntie
    ...
    # only respond when directly addressed or Azure is talking
    if not direct:
        return
    ...
    # detect Azure calling for backup
    ...
    async with message.channel.typing():
        try:
            # use vision if image attached and openai key available
            ...
            if img_match:
                ...
                # always ping Azure when replying to her
...
async def save_memory(uid, user_msg, bot_reply):
    try:
        raw = groq_chat([{"role": "user", "content":
            ...
            f"one fact per line, no bullets, no intro.\n\n"
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        print("ERROR: DISCORD_BOT token is missing. set it in your startup command.", flush=True)
    else:
